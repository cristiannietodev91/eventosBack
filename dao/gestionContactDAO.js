...
module.exports = {
    findAll: function (cb) {
        // Find all users
        models.gestioncuenta.findAll().then(gestiones => {
            cb(null, gestiones);
        });
    },
    create: function (gestion, cb) {
        ...
        return models.sequelize.transaction((t1) => {
            return models.gestioncuenta.create(gestion).then(gestion => {
                return gestion;
            });
    ...
    getById: function (idgestion, cb) {
        // Find all users
        return models.sequelize.transaction((t1) => {
            return models.gestioncuenta.findByPk(idgestion).then(gestion => {
                return gestion;
            });
        }).then(function (result) {
            if (result) {
